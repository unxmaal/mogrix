...
| IRIX bsearch nmemb=0 crash | SIGSEGV in bsearch() when nmemb=0 (GTK3 property_cache, any empty-array bsearch) | inject_compat_functions: bsearch | compat/stdlib/bsearch.c, rules/generic.yaml | IRIX libc bsearch() computes `last = base + size * (nmemb-1)` which underflows to 0xFFFFFFE0 when nmemb=0 and base=NULL. Overflow check `sltu(last < base)` fails because `0xFFFFFFE0 >= 0` unsigned. Falls through to search loop, dereferences garbage pointer. Fix: compat bsearch.c in generic.yaml inject_compat_functions — all packages get it. Symbol interposition works: compat bsearch in executable overrides libc's version for all .so in the process. |
