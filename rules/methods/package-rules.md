...
### Example: /opt/mogrix/etc/yum.repos.d
...
| `/var/lib/rpm` | rpm | rpm's database location (symlink to /opt/mogrix/lib32/sysimage/rpm) |
| `/opt/mogrix/etc/rpm` | rpm | rpm config directory |
| `/opt/mogrix/etc/rpm/macros.sqlite` | rpm | Sets sqlite as default db backend |
| `/opt/mogrix/etc/yum.repos.d` | tdnf | tdnf needs it to function |
| `/opt/mogrix/etc/yum.repos.d/mogrix.repo` | tdnf | Default bootstrap repo (points to /tmp/mogrix-repo) |
| `/opt/mogrix/var/cache/tdnf` | tdnf | tdnf's cache |
| `/opt/mogrix/etc/pki/rpm-gpg` | sgugrse-repos | GPG key storage |
