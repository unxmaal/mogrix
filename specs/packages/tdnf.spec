# IRIX/SGUG path macros (injected by mogrix)
...
# Disable auto-deps for cross-compilation
AutoReq: no
AutoProv: no
...
Source1: license.txt
License: LGPL-2.1-only AND LGPL-2.1-or-later AND GPL-2.0-only AND BSD-3-Clause
Source2: tdnf.conf
...
Source100: strerror_r.c
Source101: getopt_long.c
Source102: stpcpy.c
Source103: asprintf.c
Source104: getline.c
Source105: bsearch.c
Source106: reallocarray.c
Source107: setenv.c
Source108: strdup.c
Source109: strndup.c
Source110: strnlen.c
# Mogrix patches (IRIX compatibility)
Patch500: client-defines.sgifixes.patch
Patch501: gpgcheck-rpm419.sgifixes.patch
Patch502: tdnf-common-utils.sgifixes.patch
Patch503: tdnf.irixfixes.patch
Patch504: tdnf-vsnprintf-irix.sgifixes.patch
...
Group:      Development/Libraries
Requires:   %{name} = %{version}-%{release}
# disabled: Requires:   %{name}-automatic = %{version}-%{release}
# disabled: Requires:   %{name}-plugin-repogpgcheck = %{version}-%{release}
# disabled: Requires:   %{name}-plugin-metalink = %{version}-%{release}
# disabled: Requires:   %{name}-python = %{version}-%{release}
# disabled: Requires:   python3-pytest
# disabled: Requires:   python3-requests
Requires:   rpm-build
...
# Cross-compilation prep fixes (injected by mogrix)
$MOGRIX_ROOT/tools/safepatch CMakeLists.txt --delete-line '_XOPEN_SOURCE' --no-backup --quiet
...
export gl_cv_cc_wallow="none"
export ac_cv_header_sys_random_h="no"
export gl_cv_header_working_stdint_h="yes"
export ac_cv_func_gettimeofday="yes"
export gl_cv_func_gettimeofday_clobber="no"
export gl_cv_func_gettimeofday_posix_signature="almost"
...
export ac_cv_member_struct_sigaction_sa_sigaction="yes"
...
# IRIX source fixes are in tdnf.irixfixes.patch
# Compat library (libmogrix-compat.a) is auto-built by generic %build preamble
...
# Skip python bindings for cross-compilation
...
rm -f %{buildroot}# disabled: %{_bindir}/%{name}-automatic
...
# Skip systemd cleanup (not built)
...
%defattr(-,root,root)
# disabled: %dir %{_sysconfdir}/tdnf/pluginconf.d
# disabled: %config(noreplace) %{_sysconfdir}/tdnf/pluginconf.d/tdnfmetalink.conf
# disabled: %{_tdnfpluginsdir}/libtdnfmetalink.so

%files plugin-repogpgcheck
%defattr(-,root,root)
# disabled: %dir %{_sysconfdir}/%{name}/pluginconf.d
# disabled: %config(noreplace) %{_sysconfdir}/%{name}/pluginconf.d/tdnfrepogpgcheck.conf
# disabled: %{_tdnfpluginsdir}/libtdnfrepogpgcheck.so

%files python
%defattr(-,root,root)
# disabled: %{python3_sitelib}/*

%files automatic
%defattr(-,root,root,0755)
# disabled: %{_bindir}/%{name}-automatic
# disabled: %config(noreplace) %{_sysconfdir}/%{name}/automatic.conf
# disabled: %{_unitdir}/%{name}-automatic.timer
# disabled: %{_unitdir}/%{name}-automatic.service
# disabled: %{_unitdir}/%{name}-automatic-install.timer
# disabled: %{_unitdir}/%{name}-automatic-install.service
# disabled: %{_unitdir}/%{name}-automatic-notifyonly.timer
# disabled: %{_unitdir}/%{name}-automatic-notifyonly.service
